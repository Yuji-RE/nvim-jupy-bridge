...
`NvimJupy Debug`は、**「保存 → 同期 → 実行」** の一連フローにかかった==時間==を **区間ごとに計測して出力**している “パフォーマンス内訳ログ”である。
...
- 外部プロセス起動コスト
jupytext をCLIで叩いてるなら、**毎回** Python起動 + import が走る
